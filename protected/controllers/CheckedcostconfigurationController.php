...
class CheckedcostconfigurationController extends RController
{
    ...
    public function actionView($id)
    {
        ...
        if($model->checked_others==2){
            ...
            $this->renderPartial('receipt1',array(
                'model'=>$model,
            ));
        }
    }
...
// Uncomment the following line if AJAX validation is needed
        ...
        if(isset($_POST['CheckedCostConfiguration']))
        {
            ...
            $model->receipt_no = str_pad($last_receipt_no, 10, "0", STR_PAD_LEFT);
            $model->bus_id = $bus_id;
...
//                $time_id = $time_id_arr[$key_of_bus];             //finding time_id of selected bus. (logic = find bus_id position and using that position find time value)
            ...
            if($model->save()){
                ...
                if($checkBoxListRoute == '0'){ //queue_date, queue_time_id
                    ...
                    $route_cost->samiti_sulka=$pt*$routeCostDetails->samiti_sulka;
                    $route_cost->bhalai_kosh=$pt*$routeCostDetails->bhalai_kosh;
                    $route_cost->samrakshan=$pt*$routeCostDetails->samrakshan;
                    $route_cost->ticket=$pt*$routeCostDetails->ticket;
                    $route_cost->sahayog=$pt*$routeCostDetails->sahayog;
                    $route_cost->bima=$pt*$routeCostDetails->bima;
                    $route_cost->bibidh=$pt*$routeCostDetails->bibidh;
                    $route_cost->mandir=$pt*$routeCostDetails->mandir;
                    $route_cost->jokhim=$pt*$routeCostDetails->jokhim;
                    $route_cost->anugaman=$pt*$routeCostDetails->anugaman;
                    $route_cost->bi_bya_sulka=$routeCostDetails->bi_bya_sulka;
                    $route_cost->ma_kosh=$pt*$routeCostDetails->ma_kosh;
